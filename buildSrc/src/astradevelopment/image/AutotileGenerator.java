package astradevelopment.image;
...
/*
 * Autotile generator from https://github.com/Anuken/Mindustry/blob/master/tools/src/mindustry/tools/ImageTileGenerator.java
 */
public class AutotileGenerator {
    ...
    public static void generate(Fi autotileFile, String blockName, Fi outputDirectory) {
        Pixmap image = new Pixmap(autotileFile);

        try {
            int width = image.width, height = image.height;

            if (width % 4 != 0 || height % 4 != 0) {
                throw new IOException("Image dimensions are not divisible by 4: " + width + "x" + height);
            }
            if (width != height) {
                throw new IOException("Image is not square: " + width + "x" + height);
            }
                ...
                for (int cy = 0; cy < 4; cy++) {
                    for (int rx = 0; rx < cellSize; rx++) {
                        for (int ry = 0; ry < cellSize; ry++) {
                            int point = colorToPosition.get(layout.get((cx * cellSize + rx) * layoutWidth / (width * 3), (cy * cellSize + ry) * layoutHeight / height), -1);
            ...
            for (int i = 0; i < 47; i++) {
                ...
                outputDirectory.child(blockName + "-" + i + ".png").writePng(cropped);
                cropped.dispose();
            }
